...
public class GLTextureView extends TextureView
    ...
    private MojoTextureRenderer mojoRenderer;

    public GLTextureView(Context context, ArrayList<Uri> mediaUris) {
        super(context);

        ArrayList<FileDescriptor> mediaFds = new ArrayList<>();
        for (Uri uri : mediaUris) {
            try {
                mediaFds.add(context.getContentResolver().openFileDescriptor(uri, "r").getFileDescriptor());
            } catch (FileNotFoundException e) {
                e.printStackTrace();
            }
        }

        mojoRenderer = new MojoTextureRenderer(context, mediaFds, mediaPlayer -> {
            Log.i("MOJO", "onMediaReady");
            return null;
        });

        setEGLContextClientVersion(2);
        setRenderer(mojoRenderer);
